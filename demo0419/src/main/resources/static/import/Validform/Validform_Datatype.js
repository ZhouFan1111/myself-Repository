	...
	if($.Datatype){
		...
		$.extend($.Datatype,{
			...
			*/
            ...
            "hzExt":function(gets,obj,curform,datatype){
                /*
					附件后缀名校验
					suffix为所需验证后缀名字符串 参照默认值设置
					eg. <input type="file" name="test" datatype="hzExt" suffix=".jpg,.gif,.bmp,.png,.doc,.xls,.rar,.zip,.7z" />
				*/
                var extStr = (obj.attr("suffix")==null)?".jpg,.gif,.bmp,.png,.doc,.xls,.rar,.zip,.7z":obj.attr("suffix");
                if(gets.lastIndexOf(".")==-1)
				{
					return "无法识别文件类型！";
				}
				var extName = gets.substring(gets.lastIndexOf(".")+1).toLowerCase();
                if(extStr.indexOf(extName)!=-1)
                {
                    return true;
                }
                else
                {
                    return "只允许上传后缀名为"+extStr+"的文件！";

                }
            },

            "eq":function(gets,obj,curform,datatype){
                /*
					相同值判断
					suffix为所需验证后缀名字符串 参照默认值设置
					eg. <input type="text" name="test" datatype="eq" eqid="ipt_name|ipt_name2" errormsg="元素值不能相同!"/>
				*/
                if(obj.attr("eqid")==null)
				{
					return "未设置eqid属性值！";
				}
				var idsArr = obj.attr("eqid").split("|");
				for(var index in idsArr)
				{
					if(obj.attr("id")==idsArr[index])
					{
						continue;
					}
					if(gets==$("#"+idsArr[index]).val())
					{
						return obj.attr("errormsg")?obj.attr("errormsg"):"存在相同值！";
					}
				}
				return true;
            },

            "int":function(gets,obj,curform,datatype){
                /*
                    整数判断
                    minus 整数类型 0正整数 1包含0整数 2整数
                    eg. <input type="text" name="test" datatype="int" minus="1"  errormsg="元素值不能相同!"/>
                */
                var minus = ~~obj.attr("minus")||0;//0正整数 1包含0整数 2整数
            ...
            "float":function(gets,obj,curform,datatype){
				/*
                    浮点数判断
                    minus 整数类型 0正浮点数 1包含0浮点数 2浮点数
                    eg. <input type="text" name="test" datatype="float" minus="1" int="12" deciaml="2"  errormsg="元素值不能相同!"/>
                */
                var minus = ~~obj.attr("minus")||0;//0正浮点数 1包含0浮点数 2浮点数
                var intBit = ~~obj.attr("int")||12;
                var decimalBit = ~~obj.attr("deciaml")||2;
				if(intBit>12||intBit<1)
				{
                    intBit = 12;
				}

                if(decimalBit<1||decimalBit>10)
                {
                    decimalBit = 2;
                }
				var ipattern;
                ...
                {
                    //正浮点数
                    ipattern = "^\\d{1,"+intBit+"}\\.{0,1}\\d{0,"+decimalBit+"}$";
                }
                else
                {
                    //包含0浮点数
                ...
                var pattern = new RegExp(ipattern);
                //console.info(ipattern);
                var errStr = ",共"+(intBit+decimalBit)+"位，小数点后"+decimalBit+"位！";
                if(!pattern.test(gets))
                {
                    ...
                    {
                        //正浮点数
                        return "请输入正浮点数"+errStr;
                    }
                    else if(minus===1)
                    {
                        //包含0整数
                        return "请输入大于等于0的正浮点数"+errStr;
                    }
                    else
                    {
                        //整数
                        return "请输入浮点数"+errStr;
                    }
                }
                else
				{
                    if(minus===0&&parseFloat(gets)==0)
                    {
                        //正浮点数
                        return "请输入正浮点数"+errStr;
                    }
                    var num = new Decimal(gets);
                    $("#"+obj.attr("id")).val(num.toFixed(decimalBit));
                    return true;
				}

            },
            "passStrong":function(gets,obj,curform,datatype){
                //判断输入密码的类型
...
//密码模式
                ...
                var pass = $(obj).val();
                if (pass !="" && checkStrong(pass)<2)
                {
                    return "密码不符合安全要求，请重新设定！\r\n注意，您的新密码必须8位以上（包括8位）并且需同时包含字母和数字。 ";
                }
                return true;
            }
