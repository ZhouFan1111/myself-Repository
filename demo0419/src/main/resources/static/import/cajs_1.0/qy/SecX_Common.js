...
//GETKEYPIC class
function GetPicture() {
    var bOK = $LoadControl("3BC3C868-95B5-47ED-8686-E0E3E94EF366", "certpicture", "GetPic()", false);
    //加载签章图片控件

    if (!bOK) {
        layer.alert("load control error!");
        return null;
    } else {
        //alert("load control ok!");
        var pic = certpicture.GetPic();
        return pic;
        if(pic == "" ){
            layer.alert("没有读取到图片");
            return ;
...
function CreateXTXAppObject() {
    ...
    o.SignFile = function(strCertID, strFilePath) {
        var res = XTXAPP.SetUserConfig(8,'off');
    ...
    o.SelectFile = function()
    {
        return XTXAPP.SelectFile();
    }
...
function SelectFile()
{
    ...
    {
        return $_$CurrentObj.SelectFile();
    }
}
